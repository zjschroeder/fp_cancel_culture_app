...
## Thanks for all that extensive information! I tried to collect data from Twitter API several years ago, and it was such a pain, they blocked my account every single day haha:) It looks like a process changed a little bit, and if I try to do this again in the future, I would refer to these instructions! 
...
library(psych)
library(tictoc) # For timimg long functions #so, that is not the package for the time.sleep function to wait 1 second between each iteration, correct? that what is the purpose of timing here? 
...
#Very useful info! Thank you! I remember I never ended up making 'rtweet' work for me! 
...
## I see that you use the 'author_followers' further to count the number of followers. I wonder if the followers data could be used to look into the social network side of the question. 
```
...
#I appreciate you keeping the function without the parallel processing as well! easier to disect this way! 
...
## Very cool! How are 'mental_perception' and 'sentiment' different though? What are the possible categories in 'mental perception'? I understand it has to do what is in the dictionaries that you use, so I would benefit from some comment on what those distionaries are, and how matching the compnent of the tweet with the dictinary entry would yield this or that characteristic. 
