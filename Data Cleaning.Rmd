...
[Thanks for this note -- super helpful! I am curious about why you chose to walk through this process in R, especially since it seems like there are things that Python could do better. Perhaps a mention of that in your intro might be helpful, even if you are going to publish this app and will instead create some sort of method section inseat. 

Also, not sure if you like `p_load()` from the pacman package, but it can make loading libraries a bit more straighforward. - AG]
...
clean_twitter <- function(json_data){
  ...
  # Automatically calculating how many loops
    # why are you using 10000? either way, can set that number here
    loop_count <- 10000
   total_groups <- max(as.numeric(groupdata2::group_factor(json_data,
                                        n = loop_count,
                                        method = "greedy")))
   loops <- total_groups - 1
   loop_call_left <- c(0:loops) * loop_count + 1
   loop_call_right <- c(0:(loops)) * loop_count + loop_count
...
[quick note: not really sure what neg_positive or neg-negative is, but both only have 0s for for Will Smith and Cancel Culture. - AG]
