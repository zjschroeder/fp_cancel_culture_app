...
```{r Kanye West Import}
kanye <- rjson::fromJSON(file = here::here("data/kanyewest.json"))

clean_twitter(kanye)  
```

#### dojacatiscancelledparty

```{r doja cat}
doja <- rjson::fromJSON(file = here::here("data/dojacat.json"))
...
```{r}
lindsayellis <- rjson::fromJSON(file = here::here("data/lindsayellis.json"))

clean_twitter(lindsayellis) 
```

#### MuteRKelly

```{r}
rkelly <- rjson::fromJSON(file = here::here("data/rkelly.json"))

clean_twitter(rkelly) 
```

#### EXPORT

```{r}
save(kanye_clean, doja_clean, rkelly_clean, lindsayellis_clean, file = "celebrity_cancel.Rdata")
```
...
```{r}
bbqbecky <- rjson::fromJSON(file = here::here("data/bbqbecky.json"))

clean_twitter(bbqbecky) 
```

#### permitpatty

```{r}
permitpatty <- rjson::fromJSON(file = here::here("data/permitpatty.json"))

clean_twitter(permitpatty) 
```

#### hasjustinelanded hasjustinelandedyet justinesacco

```{r}
justinesacco <- rjson::fromJSON(file = here::here("data/justinesacco.json"))

clean_twitter(justinesacco) 
```

## Cancel Culture-wide Terms
...
# save(bbqbecky_clean, permitpatty_clean, justinesacco_clean, file = "new_cancel_data.Rdata")
```

```{r Loading data for text analysis NOT DEIDENTIFIED}
...
```{r Text Analysis}
# analyzed <- map(celeb_data, analyze_text)
...
# save(analyzed, celeb_data,
#      file = "celeb_data_shiny.RData")
save(analyzed_DI,
