...
name <- deparse(substitute(ws)) 
corp <- corpus(ws, text_field = 1)
toks <- tokens(corp, remove_punct = T)
dfmat <- dfm(toks)
ment_perc <- mental_perception(dfmat)
pos_neg <- sentiment(dfmat) 

txt_dat <- left_join(pos_neg, ment_perc, by = "doc_id")

data <- cbind(ws, txt_dat)
 # assign(
...
```{r}
ws$valence <- map2_dbl(.x = data$positive, 
                       .y = data$negative, 
                         ~ .x - .y)
```


```{r WS creating long-format for plotting purposes}
ws_long <- ws %>% 
...
```{r CC creating additional interesting parameters}
cc <- cc %>% 
  mutate(
...
```{r CC creating long-format for plotting purposes}
cc_long <- cc %>% 
 pivot_longer(cols = c("negative", "positive", "neg_positive", 
                       "neg_negative", "Experience", "Agency", 
                       "PatientEmotion", "AgentEmotion", "MindOverall"),
