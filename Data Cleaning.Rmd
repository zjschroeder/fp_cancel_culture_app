...
```{r Functions: Cleaning JSON data, eval = FALSE}
...
clean_twitter <- function(json_data){
  ...
  parallel_output <- json_to_nested_tibbles_parallel_process(json_data)
  tibble_output <- tibble(parallel_output)
  clean_output <- tidyr::unnest_auto(tibble_output, col = parallel_output)
...
In addition to Twitter's deindividuation, the other reason I have you not running the cleaning code is that it takes a long time and is very computationally demanding.
...
cancelculture_21 <- rjson::fromJSON(file = here::here("data/cc_060121_010122_lite"))
...
## Creating Dehumanization & Valence Text Analysis Scores

```{r}
corp <- corpus(data, text_field = 3)
```
